 ...
 *
 * Partial copyright 2017 Dmitry Komissarov
 */
...
static PyObject *MakeFakeStdout(dbref _dbref);
...
static PyObject *setStdout(dbref player)
{
    ...
    PyObject *stderr = MakeFakeStdout(player);
    ...
    if(PySys_SetObject("stderr", stderr) == -1) {
        MUXPy_ReportError(player, 1);
        return NULL;
    }
    return stdout;
...
static PyMethodDef fakestdout_methods[] = {
        {"write", (PyCFunction) fakestdout_write, METH_VARARGS, "Not documented"},
...
static PyMethodDef MUXObject_methods[] = {
        {"keys", (PyCFunction) MUXObject_keys, METH_VARARGS, "Not documented"},
        {NULL, NULL, 0, NULL}				/* Sentinel */
};
